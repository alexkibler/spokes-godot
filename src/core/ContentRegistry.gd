...
# Port of ContentRegistry.ts and types.ts
# Manages the database of items and rewards.
...
func get_loot_pool(count: int, run_manager: Node) -> Array:
    var pool = []
    for r in rewards.values():
        if not r.has("available") or r["available"].call(run_manager):
            pool.append(r)
...
func bootstrap() -> void:
    ...
    register_reward({
        ...
        "rarity": "uncommon",
        "apply": func(rm): rm.run_data["inventory"].append("aero_helmet")
    })
