...
static func m_to_mi(m: float) -> float:
	return m * M_TO_MI
...
static func format_fixed(val: float, decimals: int = 1) -> String:
...
static func is_close_to_integer(val: float, tolerance: float = 0.0001) -> bool:
