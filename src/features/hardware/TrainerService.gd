...
signal connected
signal disconnected
signal data_received(data: Dictionary) # Dict with power, speed, cadence
...
var _js_data_callback: Variant = null
var _js_connected_callback: Variant = null
var _js_disconnected_callback: Variant = null
...
func set_simulation_params(grade: float, crr: float, cwa: float) -> void:
	if OS.has_feature("web") and not is_mock_mode and is_connected_to_device:
		var js_call: String = "window.setFTMSGrade(" + str(grade) + ", " + str(crr) + ", " + str(cwa) + ");"
