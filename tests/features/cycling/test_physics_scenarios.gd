...
func test_physics_profile_matrix():
	var weights = [45.0, 60.0, 75.0, 90.0, 120.0, 150.0] # kg (rider only)
	var ftps = [50.0, 100.0, 150.0, 200.0, 250.0, 300.0, 350.0, 400.0, 450.0, 500.0, 600.0, 700.0, 800.0, 1000.0] # Watts
	var grades = [-0.10, -0.05, 0.0, 0.05, 0.10, 0.15] # % grade
		...
		for ftp in ftps:
			...
			for g in grades:
				...
				if int(ftp) % 100 == 0 or ftp == 50.0:
					print("%7.1f | %4d | %5.1f%% | %6.2f" % [w, ftp, g * 100.0, v_kmh])
				...
				if last_speed != 999.0:
					assert_lt(v_kmh, last_speed, "Speed should decrease as grade increases (W:%d FTP:%d G:%.1f%%)" % [w, ftp, g*100])
				last_speed = v_kmh

func test_weight_impact_on_climbs():
...
func test_aero_dominance_on_flats():
...
func test_coasting_terminal_velocities():
	var weights = [45.0, 75.0, 150.0]
	var grades = [-0.02, -0.05, -0.10] # -2%, -5%, -10%
