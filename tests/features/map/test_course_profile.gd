...
func get_simple_course() -> CourseProfile:
...
func get_surface_course() -> CourseProfile:
	return build_course_profile([
...
func test_get_grade_at_distance():
	...
	assert_eq(simple_course.get_grade_at_distance(2500.0), -0.03)
	
	# it('returns the grade of a segment at its exact start boundary')
	assert_eq(simple_course.get_grade_at_distance(1000.0), 0.05)
	
	# it('wraps around when distance exceeds total course length')
	assert_eq(simple_course.get_grade_at_distance(3000.0), 0.00)
	assert_eq(simple_course.get_grade_at_distance(3500.0), 0.00)
	assert_eq(simple_course.get_grade_at_distance(4200.0), 0.05)
	
	# it('handles negative distances')
	assert_eq(simple_course.get_grade_at_distance(-500.0), -0.03) # Should wrap to 2500m
...
func test_get_elevation_at_distance():
	...
	assert_almost_eq(simple_course.get_elevation_at_distance(1500.0), 25.0, 0.00001)
	
	# 1000 m of flat + 1000 m * 0.05 = 50 m peak
	assert_almost_eq(simple_course.get_elevation_at_distance(2000.0), 50.0, 0.00001)
	
	# After 500 m of −3% descent: 50 − 500 * 0.03 = 35 m
	assert_almost_eq(simple_course.get_elevation_at_distance(2500.0), 35.0, 0.00001)
	
	# Elevation at 3000 m is the accumulated elevation (20m), not wrapped to 0
	assert_almost_eq(simple_course.get_elevation_at_distance(3000.0), 20.0, 0.00001)
	
	# it wraps around continuously
	assert_almost_eq(simple_course.get_elevation_at_distance(6000.0), 40.0, 0.00001)
	assert_almost_eq(simple_course.get_elevation_at_distance(-3000.0), -20.0, 0.00001)
