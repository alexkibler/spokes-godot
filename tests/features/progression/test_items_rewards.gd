...
func test_item_registration() -> void:
	...
	var item: Dictionary = cr.get_item(item_id)
...
func test_loot_pool_basics() -> void:
	...
	var pool: Array[Dictionary] = cr.get_loot_pool(3)
...
func test_apply_stat_reward() -> void:
	var initial_power: float = rm.run_data["modifiers"]["powerMult"]
...
func test_equip_unequip_modifiers() -> void:
	...
	# Equip
	var success: bool = rm.equip_item(item_id)
	assert_true(success)
	...
	# Unequip
	var removed_id: String = rm.unequip_item("helmet")
