...
func test_autoplay_reward_selection_prefers_better_item():
	...
	cr.register_reward({
		...
		"label": "Pro Frame",
		"apply": func(rm): rm.add_to_inventory("pro_frame_test")
	})
	var reward_b = ContentRegistry.get_reward("item_pro_frame_test")
...
func test_autoplay_prefers_net_stat_gain():
	...
	cr.register_reward({
		...
		"apply": func(rm): rm.add_to_inventory("pro_helmet_test")
	})
	var reward_a = cr.get_reward("item_pro_helmet_test")
	var reward_b = cr.get_reward("stat_aero_1") # +2% permanent
...
func test_autoplay_reward_grand_prix():
	rm.run_data["equipped"] = {}
	var r_a = {"id": "r_a", "modifier": {"powerMult": 1.02}}
	var r_b = {"id": "r_b", "modifier": {"dragReduction": 0.03}}
	var r_c = {"id": "r_c", "modifier": {"weightMult": 0.95}}
