...
func test_calculate_acceleration_basic():
	var default_config = CyclistPhysics.get_default_config()
	
	# it('gives negative acceleration (deceleration) when coasting at 10 m/s on flat')
	var acc = CyclistPhysics.calculate_acceleration(0, 10, default_config)
	...
	acc = CyclistPhysics.calculate_acceleration(400, 5, default_config)
	...
	var downhill_config = default_config.duplicate()
	downhill_config["grade"] = -0.05
	acc = CyclistPhysics.calculate_acceleration(0, 2, downhill_config)
	...
	acc = CyclistPhysics.calculate_acceleration(0, v_terminal, downhill_config)
	...
	var speed = 8.0
	var flat = CyclistPhysics.calculate_acceleration(0, speed, default_config)
	var climb_config = default_config.duplicate()
	climb_config["grade"] = 0.08
	var climb = CyclistPhysics.calculate_acceleration(0, speed, climb_config)
	...
	speed = 5.0
	flat = CyclistPhysics.calculate_acceleration(200, speed, default_config)
	var downhill = CyclistPhysics.calculate_acceleration(200, speed, downhill_config)
	...
	acc = CyclistPhysics.calculate_acceleration(200, 0, default_config)
	...
	acc = CyclistPhysics.calculate_acceleration(0, -1, default_config)
...
func test_calculate_acceleration_modifiers():
	...
	assert_gt(
		CyclistPhysics.calculate_acceleration(200, 5, config, boost),
		CyclistPhysics.calculate_acceleration(200, 5, config, no_mod),
	...
	assert_gt(
		CyclistPhysics.calculate_acceleration(0, 10, config, slippery),
		CyclistPhysics.calculate_acceleration(0, 10, config, no_mod),
	...
	assert_gt(
		CyclistPhysics.calculate_acceleration(200, 5, config, lighter),
		CyclistPhysics.calculate_acceleration(200, 5, config, no_mod),
	...
	assert_almost_eq(
		CyclistPhysics.calculate_acceleration(300, 5, config, zero_mod),
		CyclistPhysics.calculate_acceleration(0, 5, config),
		0.00001,
