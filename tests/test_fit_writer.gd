...
func test_crc_presence():
	...
	var header_crc = data[12] | (data[13] << 8)
	assert_ne(header_crc, 0, "header CRC should be non-zero")
	
	var file_crc = data[data.size() - 2] | (data[data.size() - 1] << 8)
